...
function sampleProducer(kinesis, config) {
    ...
    function _createStreamIfNotCreated(callback) {
        var params = {
            DeliveryStreamName: 'odysseyfirehose',
        };
        kinesis.createDeliveryStream(params, function(err, data) {
            if (err) {
                if (err.code !== 'ResourceInUseException') {
                    callback(err);
                    return;
                } else {
                    log.info(util.format('%s stream is already created. Re-using it.', 'odysseyfirehose'));
                }
            } else {
                log.info(util.format("%s stream doesn't exist. Created a new stream with that name ..", 'odysseyfirehose'));
            }
    ...
    function _waitForStreamToBecomeActive(callback) {
        kinesis.describeDeliveryStream({ DeliveryStreamName: 'odysseyfirehose' }, function(err, data) {
            if (!err) {
